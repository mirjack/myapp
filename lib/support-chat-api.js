...
async function requestJson(path, options = {}) {
  ...
  const send = () => fetchWithTimeout(resolveSupportUrl(path), { ...options, headers });
  let response = await send();
  if (session !== getAuthSessionVersion()) throw new Error("Authentication session changed");
  if (response.status === 401) {
    const access = await refreshAccessToken(headers.Authorization?.replace(/^Bearer /, ""));
    if (access) { headers.Authorization = `Bearer ${access}`; response = await send(); }
  }
  if (session !== getAuthSessionVersion()) throw new Error("Authentication session changed");
