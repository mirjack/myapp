...
const PROFILE_CACHE_PREFIX = "nativeProfileSummary.v2";
const NATIVE_PROFILE_CACHE_MAX_AGE_MS = 5 * 60 * 1000;
...
function scopeFromToken(accessToken) {
  ...
  return String(accessToken).slice(-24).replace(/[^a-zA-Z0-9._-]/g, "_");
}
...
async function clearCachedNativeProfile(accessToken) {
  const key = cacheKey(accessToken);
  if (!key) return;
