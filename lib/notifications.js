...
const SUPPORT_CHAT_CHANNEL_ID = "support-chat";
...
async function getNotificationPermissionStatusAsync() {
...
async function openNotificationSettingsAsync() {
  await Linking.openSettings();
}

export async function scheduleSupportChatNotificationAsync({
  requestId,
  text,
  title = "Support chat",
} = {}) {
  if (!isNotificationsAvailable() || !String(text || "").trim()) return null;
