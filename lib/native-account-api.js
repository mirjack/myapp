import { fetchWithTimeout } from "@/lib/network-request";
import { STOREFRONT_ORIGIN, TENANT_DOMAIN } from "@/lib/runtime-config";
import { getStoredAuthTokens, parseAuthTokens, getAuthSessionVersion } from "@/lib/auth-storage";
import { refreshAccessToken } from "@/lib/auth-session";
import { getStoredLanguageCode } from "@/lib/app-preferences";
import { writeCachedNativeBrandingContacts } from "@/lib/native-branding-cache";
import {
...
async function getAccessToken() {
  ...
  return parseAuthTokens(tokensString)?.access || null;
}
...
async function requestJson(path, options = {}) {
  ...
  const makeRequest = async (token) => fetchWithTimeout(`${API_BASE_URL}${path}`, {
    ...fetchOptions,
    headers: {
      ...(await baseHeaders({
        accessToken: token,
  ...
  if (session !== getAuthSessionVersion()) throw new Error("Authentication session changed");
...
export async function fetchNativeLoyaltyTransactions() {
  const data = await requestJson("/api/v1/loyalty/transactions/", {
    method: "GET",
    isTenantScoped: true,
  });
...
async function prefetchNativeLoyaltyProfile() {
  const accessToken = await getAccessToken();
...
export async function fetchNativeBranding() {
  const data = await requestJson("/api/v1/public/branding/", {
    method: "GET",
    isTenantScoped: true,
    headers: {
  ...
  await writeCachedNativeBrandingContacts(data?.organization?.contacts || {});
  return data;
}
...
export async function verifyNativeOtp({ phoneNumber, otp }) {
  ...
  return {
    access: data?.access ?? data?.access_token ?? null,
...
export async function fetchCurrentUserProfile() {
  const accessToken = await getAccessToken();
  const session = getAuthSessionVersion();
  if (currentUserProfileRequest?.session === session) return currentUserProfileRequest.promise;
  const request = { session, promise: null };
  request.promise = (async () => {
    let data;
    try {
      data = await requestJson("/api/v1/client/users/user/");
    } catch (error) {
      if (error?.status !== 404) throw error;
      data = await requestJson("/api/v1/public/auth/me/");
    }
    if (session !== getAuthSessionVersion()) throw new Error("Authentication session changed");
    ...
    if (session === getAuthSessionVersion() && currentAccess !== accessToken) {
      await writeCachedNativeProfile(currentAccess, mappedUser);
    }
    return mappedUser;
  })();
  currentUserProfileRequest = request;
  try { return await request.promise; }
  finally { if (currentUserProfileRequest === request) currentUserProfileRequest = null; }
}

async function prefetchCurrentUserProfile() {
  const accessToken = await getAccessToken();
  if (!accessToken) return null;
