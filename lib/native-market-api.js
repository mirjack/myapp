...
import { getAuthSessionVersion } from "@/lib/auth-storage";
...
async function requestJson(path, options = {}) {
  ...
  const makeRequest = (token) => fetchWithTimeout(`${API_BASE_URL}${path}`, {
    ...fetchOptions,
    headers: {
      ...baseHeaders({
        accessToken: token,
...
export async function fetchProductList(options = {}) {
  ...
  const data = await requestJson(`/api/v1/public/catalog/products/?${query}`, { signal: options.signal });
