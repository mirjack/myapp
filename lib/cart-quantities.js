...
export function subscribeCartQuantities(listener) {
...
export function getCartQuantitiesSnapshot() {
  return quantities;
}

export function getCartQuantitiesStateSnapshot() {
  return snapshot;
}
...
export async function hydrateCartQuantities(accessToken, { force = false } = {}) {
  ...
  hydratePromise = (async () => {
    const token =
      accessToken ||
      parseTokensString(getStoredAuthTokensSync())?.access ||
      parseTokensString(await getStoredAuthTokens())?.access;
