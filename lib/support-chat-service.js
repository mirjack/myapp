...
import { scheduleSupportChatNotificationAsync } from "@/lib/notifications";
...
function createSupportChatService() {
  ...
  function preserveLocalPendingMessages(bootstrapData) {
    const currentChat = snapshot.bootstrapData?.chat;
    if (!currentChat?.requests?.length) return bootstrapData;

    const pendingByRequestId = new Map();
    for (const request of currentChat.requests) {
      const pendingMessages = (request?.messages || []).filter(
        (message) =>
          message?._sendStatus === "sending" ||
          message?._sendStatus === "failed",
      );
      if (pendingMessages.length) {
        pendingByRequestId.set(String(request.id), pendingMessages);
      }
    }

    if (!pendingByRequestId.size) return bootstrapData;

    return ensureBootstrapShape({
      ...bootstrapData,
      chat: {
        ...(bootstrapData.chat || {}),
        requests: (bootstrapData.chat?.requests || []).map((request) => {
          const pendingMessages = pendingByRequestId.get(String(request.id));
          if (!pendingMessages?.length) return request;

          const existingIds = new Set(
            (request.messages || []).map((message) => String(message.id)),
          );
          return {
            ...request,
            messages: [
  ...
  function applyIncomingPayload(payload) {
    ...
    updateBootstrapData((currentBootstrap) => {
      ...
      const customerId = currentBootstrap.chat?.sender?.id ?? null;
      const previousMessages = new Set(
        (currentBootstrap.chat?.requests || []).flatMap((request) =>
          (request?.messages || []).map((message) => String(message?.id)),
        ),
      );
      incomingMessages = (nextChat.requests || []).flatMap((request) =>
        (request?.messages || []).filter((message) => {
          const messageKey = String(message?.id ?? "");
          return (
            messageKey &&
            !previousMessages.has(messageKey) &&
            !notifiedMessageKeys.has(messageKey) &&
            Number(message?.sender?.id) !== Number(customerId) &&
            String(message?.text || "").trim()
          );
        }),
      ...
      incomingMessages.forEach((message) => {
        ...
        void scheduleSupportChatNotificationAsync({
          requestId: message.requestId ?? resolvedPayload.value?.requestId,
          text: message.text,
  ...
  async function resubscribeStreams() {
    ...
    listUnsubscribe?.();
    detailUnsubscribe?.();
    listUnsubscribe = detailUnsubscribe = null;
    ...
    const list = await subscribeSupportChatList(organizationSlug, applyIncomingPayload);
    if (sequence !== streamSequence || session !== getAuthSessionVersion()) { list?.(); return; }
    const detail = await subscribeSupportChatDetail(organizationSlug, chatId, applyIncomingPayload);
    if (sequence !== streamSequence || session !== getAuthSessionVersion()) { list?.(); detail?.(); return; }
    listUnsubscribe = list;
    detailUnsubscribe = detail;
  }
  ...
  async function hydrate(loader, nextLoading = true) {
    ...
    try {
      const nextBootstrap = preserveLocalPendingMessages(
        ensureBootstrapShape(await loader()),
      );
  ...
  let serviceSession = getAuthSessionVersion();
  subscribeAuthTokens(() => {
    if (serviceSession === getAuthSessionVersion()) return;
    serviceSession = getAuthSessionVersion();
    loadSequence += 1;
    streamSequence += 1;
    listUnsubscribe?.();
    detailUnsubscribe?.();
    listUnsubscribe = detailUnsubscribe = null;
    clearInterval(realtimeFollowTimer);
    clearTimeout(eventSyncTimer);
    stopActiveChatWatch();
    realtimeFollowTimer = eventSyncTimer = null;
    lastBootstrapAt = 0;
    activeDetailRequestId = null;
