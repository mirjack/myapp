...
export function setCurrentWebPath(path) {
  ...
  if (!nextPath.startsWith("/products/")) {
    lastNonProductWebPath = nextPath;
  }
  currentWebPath = nextPath;
  emit();
}

export function getCurrentWebPath() {
  return currentWebPath;
}

export function getLastNonProductWebPath() {
  return lastNonProductWebPath;
}
