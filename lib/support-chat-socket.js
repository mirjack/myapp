...
import { getStoredAuthTokens, parseAuthTokens, getAuthSessionVersion, subscribeAuthTokens } from "@/lib/auth-storage";
import {
...
async function getConnectHeaders() {
  ...
  const accessToken = parseAuthTokens(tokensString)?.access ?? null;
  if (!accessToken) {
...
function ensureReplySubscriptions() {
  ...
  subscribeCurrentSession(REPLY_DESTINATIONS.bootstrap, (frame) => {
  ...
  subscribeCurrentSession(REPLY_DESTINATIONS.request, (frame) => {
  ...
  subscribeCurrentSession(REPLY_DESTINATIONS.message, (frame) => {
  ...
  subscribeCurrentSession(REPLY_DESTINATIONS.errors, (frame) => {
    const payload = parsePayload(frame);
...
function createStreamSubscription(destination, binding) {
  return subscribeCurrentSession(destination, (frame) => {
    const payload = parsePayload(frame);
    binding.listeners.forEach((listener) => listener(payload));
  });
}

function subscribeCurrentSession(destination, callback) {
  const session = getAuthSessionVersion();
  return supportClient.subscribe(destination, frame => {
    if (session === getAuthSessionVersion()) callback(frame);
  });
}

let socketSession = getAuthSessionVersion();
subscribeAuthTokens(() => {
  if (socketSession === getAuthSessionVersion()) return;
  socketSession = getAuthSessionVersion();
  const oldClient = supportClient;
  supportClient = null;
  connectionPromise = null;
  replySubscriptionsReady = false;
  streamBindings.clear();
  rejectPending("Authentication session changed");
  void oldClient?.deactivate().catch(() => {});
});

async function ensureSupportSocketConnected() {
  if (!REALTIME_ENABLED) {
    ...
    getConnectHeaders().then(
      (connectHeaders) =>
        new Promise((resolve, reject) => {
          if (session !== getAuthSessionVersion()) { reject(new Error("Authentication session changed")); return; }
          let settled = false;
